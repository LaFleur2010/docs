...
<form id="feli" name="feli" method="post" action="inf_diario.php">
...
***********************************************************************************************************************/	 			
	...
	mysql_select_db("$BDATOS", $co);
		...
		$sqld = "DELETE FROM detalle_inf2 WHERE id_det = '".$_GET['id']."' ";
		mysql_query($sqld,$co);
		
		if(dbExecute($sqld))
		{
			echo"<input type='hidden' name='elimina_item' id='elimina_item' value='".$_GET['inf']."' />";
			echo"<input type='hidden' name='fecha' id='fecha' value='".$_GET['f1']."' />";
			echo"<input type='hidden' name='area' id='fecha' value='".$_GET['area']."' />";
