...
function dbConsulta($sql){
    ...
    //$db_conexion = mysql_connect("localhost", "root", "mysqlpass"); 
...
if ($_GET["accion"] == "carga_nombres") {
  echo"<select name='c1' id='c1' style='width:250px;'>";
  
  	$sql  = "SELECT * FROM tb_cargos ORDER BY nom_cargo";
	
	$rs 	= dbConsulta($sql);
	$total  = count($rs);
	echo"<option value='$nom_cargo'>$nom_cargo</option>";
			
	for ($i = 0; $i < $total; $i++)
	{
		echo "<option value='".$rs[$i]['cod_cargo']."'>".htmlentities(utf8_decode($rs[$i]['nom_cargo']))."</option>";
	}
  echo"</select>";
 exit();
}
