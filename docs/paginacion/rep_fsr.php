...
$pdf->Image('imagenes/logo_mgyt_c.jpg',13,12,23);
...
//************* TERCERA FILA ****************************
$pdf->SetFont('Arial','',8);
$pdf->Cell(50,6,"Area o contrato",1,0,'L');
$pdf->SetFont('Arial','',9);
$pdf->Cell(140,6,$desc_ger." - ".$desc_dep." - ".$nom_area,1,0,'L');
...
$pdf->Cell(156,5,"Descripcion clara y detallada del Requerimiento",1,0,'L');
...
$pdf->Cell(30,5,"",1,1,'C');
	...
	while($i < $total)
	{
		...
		$pdf->Cell(156,7,utf8_decode($desc_sol),1,0,'L');
		...
		if($det_ap_b  == 1 and $cant_b > 0 and $cant_b != $cant_det){$tipo_b = "imagenes/nook.jpg"; }
...
*******************************************************************************************************/
	$aprob_dpto = utf8_decode($aprob_dpto);
	$aprob_ger 	= utf8_decode($aprob_ger);
	
	if($aprob_dpto == "Victor Castañeda G.")	{ $a_p = $pdf->Image('imagenes/firma5.jpg',125,140,45);}
	if($aprob_dpto == "Pedro Troncoso Muñoz")	{ $a_p = $pdf->Image('imagenes/firma4.jpg',130,146,45);}
	
	if($aprob_ger  == "Claudio Peña R.")		{ $a_g = $pdf->Image('imagenes/firma2.jpg',130,160,50);}
	if($aprob_ger  == "Rodrigo Sotomayor Q.")	{ $a_g = $pdf->Image('imagenes/firma9.jpg',130,160,50);}
	if($aprob_ger  == "Claudio Guzman")			{ $a_g = $pdf->Image('imagenes/firma3.jpg',130,160,50);}
	if($aprob_ger  == "Pedro Troncoso M.")		{ $a_g = $pdf->Image('imagenes/firma4.jpg',130,160,50);}
	if($aprob_ger  == "Victor Castañeda Gomez") { $a_g = $pdf->Image('imagenes/firma5.jpg',125,160,50);}
	
	if($aprob_dpto  == "Claudio Guzman M.")		{ $a_p = $pdf->Image('imagenes/firma3.jpg',130,140,45);}
	if($aprob_dpto 	== "Eduardo Miranda Rojas")	{ $a_p = $pdf->Image('imagenes/firma6.jpg',130,140,45);}
	
	if($aprob_bod  == "Sergio Fernandez V.")	{ $a_b = $pdf->Image('imagenes/firma7.jpg',245,162,55);}
...
$pdf->Cell(80,10,"$aprob_dpto",1,0,'C');
...
$pdf->Cell(41,10,"",1,1,'L');
	...
	if($cant==0)
	{
		$ing = "INSERT INTO documentos (ruta_doc, rutac_doc, carp_doc, nom_doc, nivel_doc, sub_por, fecha_sub) values('$ruta', '$ruta_c', '$ods_sol', '$nombre', 5, 'Produccion','$fe')";
		mysql_query($ing, $co);
	}
	//$pdf->Output($ruta_c);
	$pdf->Output();
