...
***********************************************************************************************************************/	
		...
		$sQuery	=	"SELECT * FROM tb_clientes WHERE rut_cli IN (".implode(',',$aLista).") ORDER BY razon_s"; 
...
//*************************** LOGO MGYT ****************************************
$pdf->Image('imagenes/logo_rockmine_c.jpg',13,4,23);
$pdf->SetY(4);
...
**************************************************************************************************************************/
	...
	while($i < $total)
	{
		...
		$rut_cli 		= $lista_cli[$i]['rut_cli'];
		$razon_s 		= $lista_cli[$i]['razon_s'];
		$ods_sol 		= $lista_cli[$i]['ods_sol'];	
		$cc_sol 		= $lista_cli[$i]['cc_sol'];	
		$desc_sol 		= $lista_cli[$i]['desc_sol'];
		$und_med 		= $lista_cli[$i]['und_med'];
		$cant_det 		= $lista_cli[$i]['cant_det'];
		$fe_aprob_g 	= $lista_cli[$i]['fe_aprob_g'];
		$fe_en_obra 	= $lista_cli[$i]['fe_en_obra'];
		$rec_det 		= $lista_cli[$i]['rec_det'];
		$cant_rec 		= $lista_cli[$i]['cant_rec'];
		$prof_sol 		= $lista_cli[$i]['prof_sol'];
		...
		while($vrows=mysql_fetch_array($res_um))
		{
			...
			$pdf->Cell(15,6,"$rut_cli",1,0,'L',$fondo);						
			$pdf->Cell(120,6,"$razon_s",1,0,'L',$fondo);				
			$pdf->Cell(15,6,"$ods_sol",1,0,'L',$fondo);	
			$pdf->Cell(15,6,"$cc_sol",1,0,'L',$fondo);
			$pdf->Cell(20,6,cortarTexto(utf8_decode($desc_sol), 85),1,0,'L',$fondo);
