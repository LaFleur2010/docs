...
**************************************************************************/	
	...
	$worksheet->write(5,14,"VALOR TOTAL",$encabezado);
	$worksheet->write(5,15,"ING POR",$encabezado);
...
<title>Historial de Cotizaciones/Licitaciones</title>
...
{
	document.form1.action='lista_ter.php?pagina='+1;
	document.form1.submit();	
}
...
function Genera_Grafico($nula, $noadjudicado, $noestudio, $adjudicado)
{
	 // Standard inclusions   
	 include("../graficos/pChart/pData.class");
	 include("../graficos/pChart/pChart.class");
	
	 // Dataset definition 
	 $DataSet = new pData;
	 $DataSet->AddPoint(array($nula, $noadjudicado, $noestudio, $adjudicado),"Serie1");
	 $DataSet->AddPoint(array("Nula", "No adjudicado", "No estudio", "Adjudicado"),"Serie2");
	 $DataSet->AddAllSeries();
	 $DataSet->SetAbsciseLabelSerie("Serie2");
	
	 // Initialise the graph
	 $Test = new pChart(500,270);
	 $Test->drawFilledRoundedRectangle(7,7,500,270,5,240,240,240);
	 $Test->drawRoundedRectangle(5,5,500,270,5,230,230,230);
	 $Test->createColorGradientPalette(195,204,1,223,1,1,1);
	
	 // Draw the pie chart
	 $Test->setFontProperties("../graficos/Fonts/tahoma.ttf",8);
	 $Test->AntialiasQuality = 0;
	 $Test->drawPieGraph($DataSet->GetData(),$DataSet->GetDataDescription(),220,130,110,PIE_PERCENTAGE_LABEL,FALSE,60,20,5);
	 $Test->drawPieLegend(380,15,$DataSet->GetData(),$DataSet->GetDataDescription(),250,250,250);
	
	 // Write the title
	 $Test->setFontProperties("../graficos/Fonts/MankSans.ttf",10);
	 $Test->drawTitle(10,20,"Estadisticas de Cotizaciones/Licitaciones",100,100,100);
		
	/* if(is_file ("../graficos/cotizaciones.png"))
	 {
	 	unlink("../graficos/cotizaciones.png"); 
	 	$fotografia = "../graficos/Cumplimiento2.png";
	 }else{
	 	unlink("../graficos/Cumplimiento2.png"); 
	 	$fotografia = "../graficos/Cumplimiento.png";
	 }*/
	 $Test->Render("../graficos/cotizaciones.png");
}

function porcentaje($cant_cum, $cant_reg) {
    echo number_format($cant_cum / $cant_reg * 100,1,',','.');
}
?>
<form id="form1" name="form1" method="post" action="" > 
  ...
  <tr>
    ...
    <td width="498" align="center" valign="middle">HISTORIAL  DE COTIZACIONES/LICITACIONES</td>
        ...
        <tr>
          ...
          <table width="1089" border="1" bordercolor="#F2F2F2" bgcolor="#cedee1" class="txtnormal2" cellspacing="0" cellpadding="0">
        	<tr style="background:#cedee1;" class="txtnormal8">
				...
				<td width="31%" align="center">CLIENTE</td>
            ...
            <tr class="txtnormal8" align="center">
			  ...
			  ?>
			    <select name="c_num_cot" id="c_num_cot" style="font-size:9px;" onchange="evento();">
			      <?php
                //*******************************************************************************************************
					$sqlods  	= "SELECT DISTINCT num_cot FROM tb_cotizaciones WHERE estado_cot != 'Estudio' and estado_cot != 'En Espera' ORDER BY num_cot ";
			  ...
			  ?>
               ...
               <?php
                //*******************************************************************************************************
					$SqlEst  = "SELECT DISTINCT estado_cot FROM tb_cotizaciones WHERE estado_cot != 'Estudio' and estado_cot != 'En Espera' ORDER BY estado_cot ";
			  ...
			  **********************************************************************************************************************************/
			  ...
			  ?>
				...
				{
					$sql3 	= "SELECT id_cli, razon_s FROM tb_clientes WHERE id_cli = '".$_POST['c_cliente_cot']."' ORDER BY razon_s ASC";			
				...
				$SqlDist = "SELECT DISTINCT cliente_cot FROM tb_cotizaciones WHERE estado_cot != 'No Estudio' and estado_cot != 'No Adjudicado' ORDER BY cliente_cot ";
				...
				while($vrowsDist = mysql_fetch_array($resDist))
				{
					...
					$SqlRes  = "SELECT id_cli, razon_s FROM tb_clientes WHERE id_cli = '$cliente' ORDER BY razon_s ASC";	
...
$sql1	= "SELECT * FROM tb_cotizaciones WHERE estado_cot != 'Estudio' and estado_cot != 'En Espera' $query $query1 $query2 $query3 $query4 $query5 $query6 $query7 $query8 $query9 $query10 ORDER BY num_cot DESC";
...
********************************************************************************************/	
$cant_cum = 10;
if($adjudicado != 0 or $noadjudicado != 0 or $noestudio != 0 or $nula != 0)
{
	//Genera_Grafico($cant_cum, $cant_ncum);	
	Genera_Grafico($nula, $noadjudicado, $noestudio, $adjudicado);
}
/********************************************************************************************	
...
***********************************************************************************************************************/	
	...
	$sql = "SELECT * FROM tb_cotizaciones WHERE estado_cot != 'Estudio' and estado_cot != 'En Espera' $query $query1 $query2 $query3 $query4 $query5 $query6 $query7 $query8 $query9 $query10 ORDER BY num_cot DESC LIMIT ".$limitInf.",".$tamPag;
	...
	while($vrows=mysql_fetch_array($respuesta))
	{
		...
		while($vrowsc=mysql_fetch_array($resp))
		{
...
?>  
			  <tr>
			  <td height="29" colspan="8" align="center" class="txtnormal5"><?php echo "Encontrados ".$numeroRegistros." resultados"; ?></td>
              </tr>
			  <tr>
			    <td colspan="8" align="center" class="txtnormal5"><table width="1183" border="0" cellspacing="0" cellpadding="0">
			      <tr>
			        <td width="106">&nbsp;</td>
...
*****************************************************************************************************************************************************/				
            ...
            <tr>
              <td colspan="8" align="center" class="txtnormaln">
            ...
            <tr>
              <td colspan="8" align="center" class="txtnormaln">&nbsp;</td>
            </tr>
          </table>		  </td>
        </tr>
      </table>	  </td>
...
***********************************************************************************************************************/		
	...
	$sql_rep  = "SELECT * FROM tb_cotizaciones WHERE estado_cot != 'Estudio' and estado_cot != 'En Espera' $query $query1 $query2 $query3 $query4 $query5 $query6 $query7 $query8 $query9 $query10 ORDER BY num_cot DESC";
	$resp_rep = mysql_query($sql_rep, $co);
	
	$filaexcel = 6;
	while($vrows_rep=mysql_fetch_array($resp_rep))
	{
		$id_cot			= "".$vrows_rep['id_cot']."";
		...
		$worksheet->write($filaexcel,14,utf8_decode($vrows_rep['valor_cot']),$formato);
		$worksheet->write($filaexcel,15,utf8_decode($vrows_rep['ing_por_cot']),$formato);
